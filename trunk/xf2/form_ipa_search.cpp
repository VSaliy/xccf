...
const char* Cform_ipa_search::read()
{
	...
	t[ti_field_hidden] = html_input_hidden("a", n(ac_ipa_search));
	...
	t[ti_field_order] = html_select(html_option("Name", "0", order == 0) + html_option("IPA", "1", order == 1) + html_option("Posts", "2", order == 2) + html_option("Date", "3", order == 3) + html_option("Date (reverse)", "4", order == 4), "name=order");
	t[ti_field_show_domain_names] = html_input_check("show_domain_names", show_domain_names);
	return t;
}
